// Task 1: Do Phong shading in view space instead of world space
// Task 2: Implement Gouraud shading instead of Phong shading. If you did things right the lighting should look a bit off (especially the specular highlights) with the cube object
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "BasicLightingSpecular", NULL, NULL);
    if (window == NULL)
    {
    ...
    glEnable(GL_DEPTH_TEST); // enable the feel of 3D space
    ...
    std::string frsFile2 = "/src/8_BasicLighting/ShaderFiles/light_cube2.frs";
    Shader lightCubeShader(currentDir + vsFile2, currentDir + frsFile2);

    // set up vertex data (and buffer(s)) and configure vertex attributes
    // ------------------------------------------------------------------

    // in terms of local space, this vertices contain:
    //   - vertex data
    //   - normal vectors
    float vertices[] = {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //lightPos = glm::vec3(1.0f + sin(curTime) * 2.0f, sin(curTime / 2.0f) * 1.0f, lightPos.z);
