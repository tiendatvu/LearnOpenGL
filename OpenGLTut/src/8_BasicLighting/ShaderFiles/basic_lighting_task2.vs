...
/*
[datvt]
If the color is drawn in fragment shader -> it should be drawn PIXEL BY PIXEL.
If the color is drawn in vertex shader -> it should be drawn VERTEX BY VERTEX.
when the colro is drawn in vertex shader:
 - not all pixel is defined its color -> color should be interpolated from the near pixel
 -> do 3 đỉnh của tam giác được xác định chung 1 normal vector, nên khi ánh sáng chiếu vào 1 đỉnh, 3 đỉnh còn lại cũng được xác định cùng màu với đỉnh được chiếu sáng
 -> làm cho các cạnh viền của tam giác cũng có cùng màu với đỉnh được chiếu sáng
 -> khiến cho các pixel cạnh các viền và các đỉnh cũng có cảm giác được chiếu sáng
 -> gây sai lệch màu cho object
*/